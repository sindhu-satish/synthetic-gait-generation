...
class Preprocessor:
    ...
    def auto_detect(self, df: pd.DataFrame):
        if self.numeric_cols is None:
            ...
            self.numeric_cols = [c for c in self.numeric_cols if c not in ["__user_id__", "__sensor_type__", "__source_file__", "EID"]]
        if self.categorical_cols is None:
            self.categorical_cols = df.select_dtypes(include=["object", "category", "string", "bool"]).columns.tolist()
        self.categorical_cols = [c for c in self.categorical_cols if c not in ["__source_file__", "__user_id__", "__sensor_type__", "time"]]
